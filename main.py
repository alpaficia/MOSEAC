...
parser.add_argument('--increase_amount', type=float, default=0.1, help='monotony increasing amount')
    ...
    for j in range(turns):
        ...
        while not dead:
            ...
            if fixed_freq:
                ...
                a_f_eval = a[1:3]
                act_f_eval = Action_adapter(a_f_eval, max_action_f)
                act_t_eval = Action_t_relu6_adapter(a_t_eval, min_time, max_action_t)
...
def main():
    ...
    env = gymnasium.make('DynamicWorld')
    ...
    if write:
        ...
        write_path = 'runs/MOSEAC_time{}'.format("dynamicworld") + time_now
    ...
    for t in range(total_steps):
        ...
        if t < start_steps:
            ...
            a_t = Action_t_relu6_adapter_reverse(act_t, min_time, max_action_t)
            if fixed_freq:
                a = a_f
            else:
            ...
            if fixed_freq:
                ...
                a_t = a[0]
                act_f = Action_adapter(a_f, max_action_f)
                act_t = Action_t_relu6_adapter(a_t, min_time, max_action_t)
                if act_t <= min_time:
                    act_t = min_time  # We don't want the time goes to 0, which makes many troubles
                act_t = np.array([act_t])
            if current_steps < steps_per_epoch:
